...
exports.purchase = function (order_id, data, callback) {
  ...
  } else if (process.env.GATEWWAY === 'twoco' && data.key) { // Back from 2CO
    Order.model.findOne({
      _id: data.merchant_order_id,
      canceled: null
    }).populate('ticket discount').exec(function (err, order) {
      if (err) return callback(new ApiError(err.message, 'DB', true))
      if (!order) return callback(new ApiError('Invalid order.', 'INVALID_ORDER', false))
      var processed = true
      var key = process.env.TWOCO_SECRET_WORD + '' + process.env.TWOCO_SELLER_ID + '' + data.order_number + order.total.toFixed(2)
      ...
      if (!valid_date) {
        ...
        order.name = data.first_name + ' ' + data.last_name
        order.email = data.email
        order.paid = Date.now()
        order.payment.order = data.order_number
        order.payment.invoice = data.invoice_id
        order.save(function (err) {
          if (err) return callback(new ApiError(err.message, 'DB', true))
          order.sendOrderConfirmation(function (err) {
  ...
  } else if (process.env.GATEWAY === 'paypal') { // Back from PayPal
    Order.model.findOne({
      _id: data.merchant_order_id,
      canceled: null
    }).populate('ticket discount').exec(function (err, order) {
